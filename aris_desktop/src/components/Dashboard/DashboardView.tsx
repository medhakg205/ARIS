  ...
  return (
    <div className="flex flex-col h-full overflow-auto p-4 sm:p-5 gap-3.5 max-w-7xl mx-auto w-full">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 relative z-10">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <span className="text-[10px] font-semibold uppercase tracking-wider text-cyan-400 bg-cyan-500/10 px-2 py-0.5 rounded-full border border-cyan-500/20">
                Active System Target
              </span>
              {hardwareConnected && (
                <span className="text-[10px] font-medium text-emerald-400 bg-emerald-500/10 px-2 py-0.5 rounded-full border border-emerald-500/20 flex items-center gap-1.5">
          ...
          <div className="flex items-center gap-2.5 shrink-0 mt-2 sm:mt-0">
            {hardwareConnected && !activeRun && (
              <button
                ...
                className="px-4 py-2 bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-400 hover:to-teal-400 text-black font-semibold text-xs rounded-full shadow-md shadow-emerald-500/20 transition-all duration-200 flex items-center gap-2"
              >
            ...
            {activeRun && (
              <button
                ...
                className="px-4 py-2 bg-gradient-to-r from-cyan-500 to-blue-500 hover:from-cyan-400 hover:to-blue-400 text-black font-semibold text-xs rounded-full shadow-md shadow-cyan-500/20 transition-all duration-200 flex items-center gap-2"
              >
            ...
            {!hardwareConnected && !activeRun && (
              <div className="text-xs font-sans text-slate-300 bg-white/[0.04] border border-white/[0.08] px-3.5 py-1.5 rounded-full flex items-center gap-2">
      ...
      <div className="shrink-0">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3 text-[10.5px] font-sans text-slate-400">
            ...
            <span className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-cyan-400" /> Derived</span>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-3.5 shrink-0">
        ...
        {selectedBoard && (
          <div className="lg:col-span-2 bg-[#0d111a]/80 backdrop-blur-md border border-white/[0.08] rounded-2xl p-3.5 sm:p-4 shadow-sm">
        ...
        {/* Quick Actions Card */}
        <div className="bg-[#0d111a]/80 backdrop-blur-md border border-white/[0.08] rounded-2xl p-3.5 sm:p-4 shadow-sm flex flex-col justify-between">
          ...
          <div className="space-y-1.5">
            {[
              ...
              >
                ...
                <span className="text-slate-500 group-hover:text-cyan-400 transition-colors font-mono text-xs">→</span>
              </button>
            ))}
          </div>
        </div>
...
const InfoCard: React.FC<{ label: string; value: string; icon: React.ReactNode }> = ({
  ...
  <div className="bg-white/[0.025] border border-white/[0.05] rounded-xl p-2.5 flex items-center gap-2.5">
    <div className="p-1.5 rounded-lg bg-white/[0.04] text-cyan-400 shrink-0">
      {icon}
    </div>
    <div>
